...
//recherche du prjet par le nom
  ...
  // question: comment gérer la date de creation?
    const createOne = async (req,res) => {
    const {project_name, project_description, project_link, datecreated, project_date} = req.body;

    if (!project_name || !project_description || !project_link || !datecreated || !project_date) {
      res.status(400).send("You must provide all mandatories datas");
    } else {
      try {
        const [results] = await Projects.createOne({project_name, project_description, project_link, datecreated, project_date});
...
res.status(500).send(err.message);
    ...
    // question: faut-il récupérer le datecreated?
    const updateOne = async (req,res) => {
      const {id} = req.params;
      const { project_name, project_description, project_link, project_date} = req.body;
...
module.exports = { getAll, getOneById, getOneByName, createOne, updateOne, deleteOne };
